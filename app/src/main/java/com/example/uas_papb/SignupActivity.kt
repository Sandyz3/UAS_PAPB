...
import com.example.uas_papb.data.UserDao
import com.example.uas_papb.databinding.ActivitySignupBinding
import com.example.uas_papb.tool.AddOn.isNetworkAvailable
import com.example.uas_papb.tool.AddOn.isValidString
import com.example.uas_papb.tool.AddOn.notEmpty
...
class SignupActivity: AppCompatActivity() {
        ...
        if(notEmpty(usermail, userpass)) {
            ...
            if(isNetworkAvailable(baseContext)) {
                ...
                val dataUser = User(
                    ...
                    email = usermail,
                    password = userpass,
                    profileImage = "https://firebasestorage.googleapis.com/v0/b/uas-papb-68b6d.appspot.com/o/Browse%20thousands%20of%20Logo%20.jpeg?alt=media&token=8f17553a-c8c5-4ddc-b11d-0eb81fe74d09",
                    role = ROLE
                )
                Thread {
                    userDao.insert(dataUser)
    ...
    private fun sendEmailVerify(user: FirebaseUser?, usermail: String, userpass: String) {
        user?.sendEmailVerification()?.addOnCompleteListener {
            if (it.isSuccessful) {
                Toast.makeText(this, "Signup Successful Please check your email", Toast.LENGTH_LONG)
                    .show()
                val editor = sharedpref.edit()
                editor.putString(EMAIL, usermail)
                editor.putString(PASS, userpass)
